...
//        env == .production ? AppSettings.identificationCard : "uno"
        "uno"
    }
    ...
    static var url: String {
        switch env {
        case .qa:
            return "http://192.168.1.6:3333/api/v1"
        case .production:
//            return "https://\(AppSettings.identificationCard).solidusystems.mx/api/v1"
            return "https://.adf.mx/api/v1"
        }
    }
...
//            }
    ...
    static func getBodyParameters(_ endpoint: ApiEndPoint) -> [String: Any]? {
        switch endpoint {
        case .login:
            return ["uuid": UUID().uuidString, "password": PASSWORD]
